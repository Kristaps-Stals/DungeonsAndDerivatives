<template>
  <!-- background dim -->
  <div class="absolute w-full h-full opacity-50 z-10 bg-black fadeinhalf" @click="emitMessage('exit')"/>
  <!-- modal -->
  <div class="absolute w-1/3 h-1/3 z-20 anchor-center top-1/2 left-1/2 background-color rounded-2xl border-2 border-color">
    <button class="absolute anchor-center left-full btn-exit" @click="emitMessage('exit')">X</button>
    <!-- Import game -->
    ...
    <div v-if="modalType=='deletegame'">
      ...
      <div class="absolute w-2/3 h-1/4 left-1/2 top-3/4 flex" style="transform:translateX(-50%)"><button class="m-auto w-1/2 text-2xl btn-main" @click="emitMessage({message:'delete'})">DELETE</button></div>
    </div>
  </div>
</template>
